...
// Extracted from original main
static void parse_args_complete(int argc, char *argv[], struct program_args *pmyargs) {
	parse_args(argc, argv, pmyargs);

	/* apply default values if not given by user*/
	if (!pmyargs->ns) pmyargs->ns = "root\\cimv2";
	if (!pmyargs->delim) pmyargs->delim = "|";
}

static void parse_args_from_stdin(int argc, char *argv[], char **args) {
	int allocated_memory = 0, read_chars = 0, index = 0, is_space_period = true;
	char c;
	args = malloc(sizeof(char)*argc);

	// adding the first parameters
	for (int i = 0 ; i < argc -1 ; i++) {
		while ((c = fgetc()) != '"'){
			if (isspace(c)){ // disregarding white space
				if (!is_space_period) { // adding end of string char, updating local variables, skipping to next argument
					args[index][read_chars + 1] = '\0';
					read_chars = 0;
					is_space_period = true;
					break;
				}
				is_space_period = true;
				continue;
			}
			if (is_space_period){ // first non white space char - allocating memory for new parameter and updating variables
				args[index] = malloc(sizeof(char)*2);
				index++;
				allocated_memory = 2;
			}
			if (read_chars + 1 >= allocated_memory) { // checking if more memory is needed to be allocated
				allocated_memory *= 2;
				args[index] = realloc(args[index], sizeof(char)*allocated_memory);
			}
			// adding the new char
			args[index][read_chars] = c;
			read_chars++;
		}
	}
	// adding the query
...
// Extracted from original main
static void init_dcerpc_and_dcom() {
	dcerpc_init();
	dcerpc_table_init();
...
// Extracted from original main
static void init_context(struct com_context *ctx){
	com_init_ctx(&ctx, NULL);
	dcom_client_init(ctx, cmdline_credentials);
}

// Extracted from original main
int print_results_from_enumerator(struct program_args args, struct com_context *ctx, struct IEnumWbemClassObject *pEnum){
	WERROR result;
	NTSTATUS status;
	uint32_t cnt = 5, ret;
	do {
...
int main(int argc, char **argv)
{
	...
	if (argc == 1) { //stdin option
		parse_args_from_stdin(argc, argv, new_argv);
		parse_args_complete(argc, new_argv, &args);
	}
	else { // regular option
		parse_args_complete(argc, argv, &args);
	}

	init_dcerpc_and_dcom();
	...
	int print_result;
	print_result = print_results_from_enumerator(args, ctx, pEnum);
